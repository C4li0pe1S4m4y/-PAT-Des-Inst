...
{
    public class C2Accion
    {
        public string query;

        public DataTable C2Read(string fadn, string ano, int estado)
        {
            ...
            if (estado > 1)
            {
                ...
                "UNION ALL " +
                "SELECT cpe.idc2 as numero, fc.nombre as formato, " +
        ...
        public DataTable C2TotalRead(string fadn, string ano, int estado)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                ...
                "FROM pat_c2 cpe WHERE fadn = '{0}' AND ano = '{1}' AND fkestado = '{2}'; ", fadn, ano, estado);
            }
            else
            {
                query = String.Format("SELECT cpe.idc2 AS numero, SUM(cpe.presupuesto) AS total " +
                "FROM pat_c2 cpe WHERE fadn = '{0}' AND ano = '{1}' AND fkestado IN(1, 2); ", fadn, ano, estado);
            }
        ...
        public DataTable C2Create(ModeloCPE objCrear)
        {
            var mysql = new DBConnection.ConexionMysql();
            DataTable dt = new DataTable();
            query = String.Format("INSERT INTO pat_c2 (ene_abr, may_ago, sep_dic, presupuesto, fkformato_c, fadn, ano, anual, fkestado) " +
        ...
        public DataTable C2Seleccionar(int id)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();

            query = String.Format("SELECT cpe.idc2 as numero, fc.nombre as formato, " +
            "cpe.ene_abr as mes1, cpe.may_ago as mes2, cpe.sep_dic as mes3, " +
        ...
        public Boolean C2Update(ModeloCPE o, int id, int estado)
        {
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2 SET fkestado = '{0}' WHERE idc2 = '{1}'",
                estado, id);
            }
            else
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2 SET ene_abr = '{1}', may_ago = '{2}', " +
                "sep_dic = '{3}', anual = '{4}', presupuesto = '{5}' WHERE idc2 = '{6}';",
        ...
        public int C2Estado(int id)
        {
            try
            {
                var mysql = new DBConnection.ConexionMysql();
                query = String.Format("SELECT fkestado FROM pat_c2 " +
                "WHERE idc2 = '{0}'", id);
                mysql.AbrirConexion();
        ...
        public bool ExisteC2(int formato, string fadn, string anio)
        {
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT fkformato_c FROM pat_c2 WHERE fkformato_c = '{0}' AND ano = '{1}' AND fadn = '{2}';", formato, anio, fadn);
            mysql.AbrirConexion();
            MySqlCommand consulta = new MySqlCommand(query, mysql.conectar);
            MySqlDataReader buscar = consulta.ExecuteReader();
            using (buscar)
