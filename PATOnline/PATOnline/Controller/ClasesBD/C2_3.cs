...
namespace PATOnline.Controller.ClasesBD
{
    public class C2_3
    {
        ...
        public DataTable C2_3Read(string fadn, string ano, int estado)
        {
        ...
        public DataTable C2_3TotalRead(string fadn, string ano, int estado)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                ...
                "FROM pat_c2_3 p WHERE p.fadn = '{0}' AND p.ano = '{1}' AND p.fkestado = '{2}';", fadn, ano, estado);
            }
            else
            {
                query = String.Format("SELECT p.idc2_3 AS numero, SUM(p.presupuesto) AS total " +
                "FROM pat_c2_3 p WHERE p.fadn = '{0}' AND p.ano = '{1}' AND p.fkestado IN (1,2);", fadn, ano, estado);
            }
        ...
        public DataTable C2_3Create(ModeloC1 objCrear)
        {
            var mysql = new DBConnection.ConexionMysql();
            DataTable dt = new DataTable();
            query = String.Format("INSERT INTO pat_c2_3 (codigo, resultado, descripcion, registro, inicio_dia, inicio_mes, fin_dia, " +
        ...
        public DataTable C2_3Seleccionar(int id)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();

            query = String.Format("SELECT c.idc2_3, c.codigo, c.fkactividad_pat, c.presupuesto, c.resultado, c.registro, " +
        ...
        public Boolean C2_3Update(ModeloC1 o, int id, int estado)
        {
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2_3 SET fkestado = '{0}' WHERE idc2_3 = '{1}'",
                estado, id);
            }
            else
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2_3 c SET c.fkactividad_pat = '{0}', c.presupuesto = '{1}', " +
                ...
                "WHERE c.idc2_3 = '{11}';", o.fkactividad, o.prespuesto, o.resultado, o.registro,
        ...
        public int C2_3Estado(int id)
        {
            try
            {
                var mysql = new DBConnection.ConexionMysql();
                query = String.Format("SELECT fkestado FROM pat_c2_3 " +
                "WHERE idc2_3 = '{0}'", id);
                mysql.AbrirConexion();
        ...
        public int C2_3Codigo(string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
            mysql.AbrirConexion();
        ...
        public bool C2_3ExisteCodigo(string codigo, string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT codigo FROM pat_c2_3 WHERE codigo='{0}' AND fadn='{1}' AND ano='{2}';", codigo, fadn, ano);
            mysql.AbrirConexion();
            MySqlCommand consulta = new MySqlCommand(query, mysql.conectar);
            MySqlDataReader buscar = consulta.ExecuteReader();
            using (buscar)
        ...
        public double C2_3Presupuesto(string fadn, string ano, int estado)
        {
            var mysql = new DBConnection.ConexionMysql();
            mysql.AbrirConexion();
            if (estado > 1)
            {
                ...
                "AND f.nombre = 'C2.3 Actividad Física y Deporte para Personas con Discapacidad' AND cpe.fkestado = '{2}';", fadn, ano, estado);
            }
            else
            {
                query = String.Format("SELECT cpe.presupuesto AS numero FROM pat_c2 cpe " +
                "INNER JOIN admin_formato_c f ON f.idformato_c = cpe.fkformato_c " +
                "WHERE cpe.fadn = '{0}' AND cpe.ano = '{1}' " +
                "AND f.nombre = 'C2.3 Actividad Física y Deporte para Personas con Discapacidad' AND cpe.fkestado IN (1,2);", fadn, ano, estado);
            }
