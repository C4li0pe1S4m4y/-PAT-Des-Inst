...
namespace PATOnline.Controller.ClasesBD
{
    public class C2_2
    {
        ...
        public DataTable C2_2Read(string fadn, string ano, int estado)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                query = String.Format("SELECT p.idc2_2 AS numero, p.codigo AS codigo, a.nombre AS actividad, " +
                ...
                "FROM pat_c2_2 p INNER JOIN admin_actividad_pat a ON a.idactividad_pat = p.fkactividad_pat " +
        ...
        public DataTable C2_2TotalRead(string fadn, string ano, int estado)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                query = String.Format("SELECT p.idc2_2 AS numero, SUM(p.presupuesto) AS total " +
                "FROM pat_c2_2 p WHERE p.fadn = '{0}' AND p.ano = '{1}' AND p.fkestado = '{2}';", fadn, ano, estado);
            }
            else
            {
                query = String.Format("SELECT p.idc2_2 AS numero, SUM(p.presupuesto) AS total " +
                "FROM pat_c2_2 p WHERE p.fadn = '{0}' AND p.ano = '{1}' AND p.fkestado IN (1,2);", fadn, ano, estado);
            }
        ...
        public DataTable C2_2Create(ModeloC1 objCrear)
        {
            var mysql = new DBConnection.ConexionMysql();
            DataTable dt = new DataTable();
            query = String.Format("INSERT INTO pat_c2_2 (codigo, resultado, descripcion, registro, inicio_dia, inicio_mes, fin_dia, " +
        ...
        public DataTable C2_2Seleccionar(int id)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();

            query = String.Format("SELECT c.idc2_2, c.codigo, c.fkactividad_pat, c.presupuesto, c.resultado, c.registro, " +
        ...
        public Boolean C2_2Update(ModeloC1 o, int id, int estado)
        {
            var mysql = new DBConnection.ConexionMysql();
            if (estado > 1)
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2_2 SET fkestado = '{0}' WHERE idc2_2 = '{1}'",
                estado, id);
            }
            else
            {
                query = String.Format("SET SQL_SAFE_UPDATES=0; " +
                "UPDATE pat_c2_2 c SET c.fkactividad_pat = '{0}', c.presupuesto = '{1}', " +
                ...
                "WHERE c.idc2_2 = '{11}';", o.fkactividad, o.prespuesto, o.resultado, o.registro,
        ...
        public int C2_2Estado(int id)
        {
            try
            {
                var mysql = new DBConnection.ConexionMysql();
                query = String.Format("SELECT fkestado FROM pat_c2_2 " +
                "WHERE idc2_2 = '{0}'", id);
                mysql.AbrirConexion();
        ...
        public int C2_2Codigo(string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
            mysql.AbrirConexion();
        ...
        public bool C2_2ExisteCodigo(string codigo, string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
        ...
        public double C2_2Presupuesto(string fadn, string ano, int estado)
        {
            var mysql = new DBConnection.ConexionMysql();
            mysql.AbrirConexion();
            if (estado > 1)
            {
                ...
                "AND f.nombre = 'C2.2 Programa de Apoyo a Deportistas' AND cpe.fkestado = '{2}';", fadn, ano, estado);
            }
            else
            {
                query = String.Format("SELECT cpe.presupuesto AS numero FROM pat_c2 cpe " +
                "INNER JOIN admin_formato_c f ON f.idformato_c = cpe.fkformato_c " +
                "WHERE cpe.fadn = '{0}' AND cpe.ano = '{1}' " +
                "AND f.nombre = 'C2.2 Programa de Apoyo a Deportistas' AND cpe.fkestado IN (1,2);", fadn, ano, estado);
            }
