...
namespace PATOnline.Controller.Read
{
    public class ReadP2
    {
        ...
        public DataTable Part9Read()
        {
            ...
            "WHERE subpadre = '0' AND idpadre = '1' OR idprograma_ac = '1';");
            mysql.AbrirConexion();
        ...
        public DataTable Part10Read()
        {
            ...
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT idprograma_ac AS idnumero1, proyeccion_egresos FROM admin_programa_ac " +
            "WHERE subpadre = '0' AND idpadre = '2' OR idprograma_ac = '2';");
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(dt);
            mysql.CerrarConexion();
        ...
        public DataTable TotalP2Read(string fadn, string anio)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT idp2 AS idnumero2, SUM(col_uno) AS total1, SUM(col_dos) AS total2, " +
            "SUM(col_tres) AS total3, SUM(col_cuatro) AS total4 " +
        ...
        public DataTable SubyTotalP2Read(int id, string fadn, string anio)
        {
            ...
            "WHERE idpadre = '1' AND subpadre = '{0}' AND fadn = '{1}' AND anio = '{2}' AND idprograma_ac != '1';", id, fadn, anio);
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(dt);
            mysql.CerrarConexion();
