...
namespace PATOnline.Controller.Read
{
    public class ReadP1
    {
        ...
        public DataTable Part11Read(int id1, string fadn, string anio)
        {
            ...
            if(id1 == 3)
            {
                add = "WHERE idpadre = '1' AND subpadre = '{0}' OR idingreso_corriente = '{0}' OR idingreso_corriente = '1' GROUP BY (ic.idingreso_corriente);";
            }
            if(id1 >= 4 || id1 <= 7)
            {
                add = "WHERE idpadre = '1' AND subpadre = '{0}' OR idingreso_corriente = '{0}' GROUP BY (ic.idingreso_corriente);";
            }
            if(id1 == 8)
            {
                add = "WHERE idpadre = '2' AND subpadre = '{0}' OR idingreso_corriente = '{0}' OR idingreso_corriente = '2' GROUP BY (ic.idingreso_corriente);";
            }
            query = String.Format("SELECT ic.idingreso_corriente AS idnumero2, ic.codigo AS codigo, " +
            "ic.nombre AS nombre, CONCAT('Q ',p.col_uno) AS monto1, CONCAT('Q ',p.col_dos) AS monto2, CONCAT('Q ',p.col_tres) AS monto3 " +
