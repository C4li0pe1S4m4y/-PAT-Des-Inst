...
namespace PATOnline.Controller.Read
{
    ...
    {
        public string query = "";
        public DataTable C1Read(string fadn, string ano, int id)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT cpe.idc1 AS numero, fc.nombre AS formato, cpe.ene_abr AS bimestre1, cpe.may_ago AS bimestre2, " +
        ...
        public DataTable C1TotalRead(string fadn, string ano)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT cpe.idc1 AS numero, SUM(cpe.presupuesto) AS total " +
            "FROM pat_c1 cpe WHERE fadn = '{0}' AND ano = '{1}'; ", fadn, ano);
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(dt);
            mysql.CerrarConexion();
