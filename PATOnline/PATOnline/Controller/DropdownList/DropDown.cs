...
namespace PATOnline.Controller.DropdownList
{
    public class DropDown
    {
        ...
        public void Drop_FADN(DropDownList drop, string nombre)
        {
            ...
            {
                add = " WHERE dbsecretaria.sg_fadn.nombre LIKE '%{0}%' AND dbsecretaria.sg_fadn.id_fand != '47';";
            }
            if (nombre == "Asociación" || nombre == "Confederación")
            {
                add = " WHERE dbsecretaria.sg_fadn.nombre LIKE '%{0}%';";
            }
            if(nombre == "all")
            {
                add = " ;";
            }
            string query = String.Format("SELECT nombre FROM dbsecretaria.sg_fadn" + add, nombre);
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(tabla);
            mysql.CerrarConexion();
        ...
        public void Drop_Menu(DropDownList ddl)
        {
            ...
            ddl.Items.Clear();
            ddl.AppendDataBoundItems = true;
            ddl.Items.Add("-- Seleccionar Menu --");
            ddl.Items[0].Value = "-1";
            ddl.DataSource = dt;
            ddl.DataTextField = "nombre";
