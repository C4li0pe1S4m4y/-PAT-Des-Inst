...
{
    public partial class IndexC2 : System.Web.UI.Page
    {
        DropDown drop = new DropDown();
        C2Accion pat = new C2Accion();
        ...
        protected void CargarGrid()
        {
            ...
            switch (Session["Rol"].ToString())
            {
                case "Usuario Interno de FADN":
                    gridFADN.DataSource = pat.C2Read(Session["Federacion"].ToString(), year, 0);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.C2TotalRead(Session["Federacion"].ToString(), year, 0);
                    gridFADNTotal.DataBind();

                    DataTable mostrar5 = new DataTable();
                    mostrar5 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "C2");
                    ...
                    for (int j = 0; j < mostrar5.Rows.Count; j++)
                    {
                        switch (mostrar5.Rows[j][0].ToString())
                        {
                            ...
                            case "Crear":
                                crearC2New.Visible = true;
                                nuevoC2.Visible = true;
                                break;
                        }
                    }
                    break;
                ...
                case "Usuario CE de FADN":
                    gridFADN.DataSource = pat.C2Read(Session["Federacion"].ToString(), year, 3);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.C2TotalRead(Session["Federacion"].ToString(), year, 3);
                    gridFADNTotal.DataBind();

                    DataTable mostrar4 = new DataTable();
                    mostrar4 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "C2");
                ...
                case "Técnico Acompañamiento":
                    ...
                    gridFADN.DataSource = pat.C2Read(Session["FederacionAsignada"].ToString(), year, 6);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.C2TotalRead(Session["FederacionAsignada"].ToString(), year, 6);
                    gridFADNTotal.DataBind();

                    DataTable mostrar2 = new DataTable();
                    mostrar2 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "C2");
                ...
                case "Técnico Evaluación":
                    ...
                    gridFADN.DataSource = pat.C2Read(Session["FederacionAsignada"].ToString(), year, 9);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.C2TotalRead(Session["FederacionAsignada"].ToString(), year, 9);
                    gridFADNTotal.DataBind();

                    DataTable mostrar3 = new DataTable();
                    mostrar3 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "C2");
        ...
        protected void nuevoC2_Click(object sender, EventArgs e)
        {
            drop.Drop_FormatoC(dropCrearFormato, 3);
            mostrarCrearC2.Visible = true;
        }

        protected void dropCrearFormato_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (pat.ExisteC2(int.Parse(dropCrearFormato.SelectedValue), Session["Federacion"].ToString(), year) == true)
            {
        ...
        protected void cancelCrearC2_Click(object sender, EventArgs e)
        {
            mostrarCrearC2.Visible = false;
        }

        protected void crearC2_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        protected void cancelEditC2_Click(object sender, EventArgs e)
        {
            mostrarEditC2.Visible = false;
        }

        protected void editC2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                pat.C2Update(modelo, int.Parse(idEditCPE.Text), 0);
                CargarGrid();
                mostrarEditC2.Visible = false;
        ...
        protected void guardarObservacionRechazo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                pat.C2Update(modelo, int.Parse(idIntroObservacionRechazo.Text), 13);
                crearObservacionRechazo.Visible = false;
                CargarGrid();
        ...
        protected void btObservacionSinRechazo_Click(object sender, EventArgs e)
        {
            try
            {
                obseracion.id5 = int.Parse(idIntroObservacionSinRechazo.Text);
                obseracion.observacion = txtCrearObservacionSinRechazo.Value;
                obseracion.usuario = id.idUsuario(Convert.ToString(Session["Usuario"]));
                obs.observacionCreateAcompaniamiento(obseracion);
                pat.C2Update(modelo, int.Parse(idIntroObservacionSinRechazo.Text), 9);
                crearObservacionSinRechazo.Visible = false;
                CargarGrid();
                ScriptManager.RegisterStartupScript(this, typeof(string), "Mensaje", "swal('¡Completo!', 'La informaicón fue ingresada', 'success');", true);
