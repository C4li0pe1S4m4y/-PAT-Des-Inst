...
namespace PATOnline.Views.CPE
{
    public partial class indexCPE : System.Web.UI.Page
    {
        ...
        protected void CargarGrid()
        {
            ...
            crearCPENew.Visible = false;
            mostrarCrearCPE.Visible = false;
            mostrarEditCPE.Visible = false;
            crearObservacionSinRechazo.Visible = false;
            crearObservacionRechazo.Visible = false;
            crearObservacionSinRechazo.Visible = false;
            mostrarObservacion.Visible = false;

            crearCPE.Visible = false;
            editCPE.Visible = false;
            btPDF.Visible = false;
            ...
            switch (Session["Rol"].ToString())
            {
                case "Usuario Interno de FADN":
                    gridFADN.DataSource = pat.CPERead(Session["Federacion"].ToString(), year, 0);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.CPETotalRead(Session["Federacion"].ToString(), year, 0);
                    gridFADNTotal.DataBind();

                    DataTable mostrar5 = new DataTable();
                    mostrar5 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "CPE");
                    ...
                    for (int j = 0; j < mostrar5.Rows.Count; j++)
                    {
                        switch (mostrar5.Rows[j][0].ToString())
                        {
                            ...
                            case "Crear":
                                crearCPENew.Visible = true;
                                nuevoCPE.Visible = true;
                                break;
                        }
                    }
                    break;
                ...
                case "Usuario CE de FADN":
                    gridFADN.DataSource = pat.CPERead(Session["Federacion"].ToString(), year, 3);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.CPETotalRead(Session["Federacion"].ToString(), year, 3);
                    gridFADNTotal.DataBind();

                    DataTable mostrar4 = new DataTable();
                    mostrar4 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "CPE");
                ...
                case "Técnico Acompañamiento":
                    ...
                    gridFADN.DataSource = pat.CPERead(Session["FederacionAsignada"].ToString(), year, 6);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.CPETotalRead(Session["FederacionAsignada"].ToString(), year, 6);
                    gridFADNTotal.DataBind();

                    DataTable mostrar2 = new DataTable();
                    mostrar2 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "CPE");
                ...
                case "Técnico Evaluación":
                    ...
                    gridFADN.DataSource = pat.CPERead(Session["FederacionAsignada"].ToString(), year, 9);
                    gridFADN.DataBind();
                    gridFADNTotal.DataSource = pat.CPETotalRead(Session["FederacionAsignada"].ToString(), year, 9);
                    gridFADNTotal.DataBind();

                    DataTable mostrar3 = new DataTable();
                    mostrar3 = boton.BotonReadUsuario(Session["Usuario"].ToString(), "CPE");
        ...
        protected void nuevoCPE_Click(object sender, EventArgs e)
        {
            drop.Drop_FormatoC(dropCrearFormato, 1);
            mostrarCrearCPE.Visible = true;
        }

        protected void dropCrearFormato_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (pat.ExisteCPE(int.Parse(dropCrearFormato.SelectedValue), Session["Federacion"].ToString(), year) == true)
            {
        ...
        protected void cancelCrearCPE_Click(object sender, EventArgs e)
        {
            mostrarCrearCPE.Visible = false;
        }

        protected void crearCPE_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        protected void cancelEditCPE_Click(object sender, EventArgs e)
        {
            mostrarEditCPE.Visible = false;
        }

        protected void editCPE_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                pat.CPEUpdate(modelo, int.Parse(idEditCPE.Text), 0);
                CargarGrid();
                mostrarEditCPE.Visible = false;
        ...
        protected void guardarObservacionRechazo_Click(object sender, EventArgs e)
        {
            try
            {
                obseracion.id16 = int.Parse(idIntroObservacionRechazo.Text);
                ...
                pat.CPEUpdate(modelo, int.Parse(idIntroObservacionRechazo.Text), 13);
                crearObservacionRechazo.Visible = false;
                CargarGrid();
        ...
        protected void btcrearObservacionRechazo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Session["Rol"].ToString() == "Usuario CE de FADN" || Session["Rol"].ToString() == "Técnico Evaluación")
                {
                    ...
                    pat.CPEUpdate(modelo, int.Parse(idIntroObservacionRechazo.Text), 2);
                    crearObservacionRechazo.Visible = false;
                    CargarGrid();
                }
        ...
        protected void btObservacionSinRechazo_Click(object sender, EventArgs e)
        {
            try
            {
                obseracion.id16 = int.Parse(idIntroObservacionSinRechazo.Text);
                obseracion.observacion = txtCrearObservacionSinRechazo.Value;
                obseracion.usuario = id.idUsuario(Convert.ToString(Session["Usuario"]));
                obs.observacionCreateAcompaniamiento(obseracion);
                pat.CPEUpdate(modelo, int.Parse(idIntroObservacionSinRechazo.Text), 9);
                crearObservacionSinRechazo.Visible = false;
                CargarGrid();
                ScriptManager.RegisterStartupScript(this, typeof(string), "Mensaje", "swal('¡Completo!', 'La informaicón fue ingresada', 'success');", true);
        ...
        protected void gridFADN_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            mostra = boton.BotonReadUsuario(Session["Usuario"].ToString(), "CPE");
                ...
                if (int.Parse(e.Row.Cells[0].Text) > 0)
                {
                    for (int j = 0; j < mostra.Rows.Count; j++)
                    {
                        switch (mostra.Rows[j][0].ToString())
                        {
                            ...
                            case "Editar":
                                if (pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 2)
                                {
                            ...
                            case "Ver":
                                if (obs.ObservacionCEFADNExiste(int.Parse(e.Row.Cells[0].Text), 17) == true
                                            || obs.ObservacionAcompaniamientoExiste(int.Parse(e.Row.Cells[0].Text), 17) == true
                                            || obs.ObservacionEvaluadorExiste(int.Parse(e.Row.Cells[0].Text), 17) == true)
                                {
                            ...
                            case "Eliminar":
                                if (pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 1 || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 2)
                                {
                            ...
                            case "Enviar":
                                if (pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 1
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 2
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 3
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 6
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 9)
                                {
                            ...
                            case "Aprobar":
                                if (pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 3 || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 9)
                                {
                            ...
                            case "Observación":
                                if (pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 3
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 6
                                        || pat.CPEEstado(int.Parse(e.Row.Cells[0].Text)) == 9)
                                {
        ...
        protected void gridFADN_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Editar")
            {
                ...
                {
                    case "Usuario Interno de FADN":
                        drop.Drop_FormatoC(dropEditFormato, 1);
                        data = pat.CPESeleccionar(int.Parse(row.Cells[0].Text));
                        ...
                        mostrarEditCPE.Visible = true;
                        break;

                    case "Técnico Evaluación":
                        data = obs.observacionMostrarAcompaniamiento(int.Parse(row.Cells[0].Text), 17);
            ...
            {
                listObservacionesFADN.DataSource = obs.observacionMostrarFADN(int.Parse(row.Cells[0].Text), 17);
                listObservacionesAcompaniamiento.DataSource = obs.observacionMostrarAcompaniamiento(int.Parse(row.Cells[0].Text), 17);
                listObservacionesEvaluacion.DataSource = obs.observacionMostrarEvaluador(int.Parse(row.Cells[0].Text), 17);
            ...
            if (e.CommandName == "Aprobar")
            {
                ...
                {
                    case "Usuario CE de FADN":
                        obseracion.id16 = int.Parse(row.Cells[0].Text);
                        obseracion.usuario = id.idUsuario(Convert.ToString(Session["Usuario"]));
                        obs.observacionCreateFADN(obseracion);
                        pat.CPEUpdate(modelo, int.Parse(row.Cells[0].Text), 6);
                        break;
                    ...
                    case "Técnico Evaluación":
                        ...
                        obseracion.usuario = id.idUsuario(Convert.ToString(Session["Usuario"]));
                        obs.observacionCreateEvaluador(obseracion);
                        pat.CPEUpdate(modelo, int.Parse(row.Cells[0].Text), 13);
                        break;
                }
            ...
            if (e.CommandName == "Enviar")
            {
                ...
                {
                    case "Usuario Interno de FADN":
                        pat.CPEUpdate(modelo, int.Parse(row.Cells[0].Text), 3);
                        break;
                    ...
                    case "Técnico Acompañamiento":
                        ...
                        obseracion.usuario = id.idUsuario(Convert.ToString(Session["Usuario"]));
                        obs.observacionCreateAcompaniamiento(obseracion);
                        pat.CPEUpdate(modelo, int.Parse(row.Cells[0].Text), 9);
                        break;
                }
            ...
            {
                pat.CPEUpdate(modelo, int.Parse(row.Cells[0].Text), 12);
                CargarGrid();
            }
            gridFADN.Columns[0].Visible = false;
        }
