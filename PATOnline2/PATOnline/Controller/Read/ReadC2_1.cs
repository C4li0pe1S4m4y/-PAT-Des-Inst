...
{
    public class ReadC2_1
    {
        public string query = "";
        public DataTable C2_1Read(string fadn, string ano)
        {
        ...
        public DataTable C2_1TotalRead(string fadn, string ano)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT p.idc2_1 AS numero, SUM(p.presupuesto) AS total " +
            "FROM pat_c2_1 p WHERE fadn = '{0}' AND ano = '{1}'; ", fadn, ano);
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(dt);
            mysql.CerrarConexion();
