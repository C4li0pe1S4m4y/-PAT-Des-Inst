...
{
    public class ReadC2_2
    {
        public string query = "";
        public DataTable C2_2Read(string fadn, string ano)
        {
        ...
        public DataTable C2_2TotalRead(string fadn, string ano)
        {
            DataTable dt = new DataTable();
            var mysql = new DBConnection.ConexionMysql();
            query = String.Format("SELECT p.idc2_2 AS numero, SUM(p.presupuesto) AS total " +
            "FROM pat_c2_2 p WHERE fadn = '{0}' AND ano = '{1}'; ", fadn, ano);
            mysql.AbrirConexion();
            MySqlDataAdapter consulta = new MySqlDataAdapter(query, mysql.conectar);
            consulta.Fill(dt);
            mysql.CerrarConexion();
