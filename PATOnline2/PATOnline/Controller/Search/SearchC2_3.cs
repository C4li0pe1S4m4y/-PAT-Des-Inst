...
namespace PATOnline.Controller.Search
{
    public class SearchC2_3
    {
        ...
        public bool ExisteCodigoC2_3(string codigo, string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
        ...
        public double PresupuestoC2_3(string fadn, string ano)
        {
            var mysql = new DBConnection.ConexionMysql();
            mysql.AbrirConexion();
            ...
            "AND f.nombre = 'C2.3 Actividad Física y Deporte para Personas con Discapacidad';", fadn, ano);
            MySqlCommand cmd = new MySqlCommand(query, mysql.conectar);
            MySqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
